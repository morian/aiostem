...
class CallError(OSError):
    """
    Error response when making a system call.

    This is an **OSError** subclass
    ...
    __slots__ = ('command', 'exit_status', 'runtime', 'stderr', 'stdout')
    ...
    def __str__(self):
        """
        Return CallError's message as a string.

        :returns str: returns msg as a string
        """
        return f'{self.command} returned exit status {self.exit_status}'


def is_windows():
    """
    Return weather or not we are running Windows.
...
def is_mac():
    """
    Return weather or not we are running Mac OSX.
...
def is_gentoo():
    """
    Return weather or not we are running Gentoo.
...
def is_slackware():
    """
    Return n weather or not we are running on a Slackware system.
...
def is_bsd():
    """
    Return if we are within the BSD family of operating systems. T.

    This currently recognizes Macs, FreeBSD, and OpenBSD but may
    be expanded later.
...
async def call(
    ...
    """
    Issues a command in a subprocess, blocking until completion and returning the results.

    This is not actually ran in a shell so pipes and other shell syntax
    are not permitted.
    ...
    * **asyncio.TimeoutError** if the timeout is reached without a default
    ...
    command_list = command.split(' ') if isinstance(command, str) else list(map(str, command))
...
async def is_avalible(command: str, cached: bool = True):
    """
    Return if a command is available or not.

    If more than one command is present (for instance "ls -a | grep foo") then this
    ...
    return await asyncio.to_thread(
        _is_available if not cached else lru_is_avalible, command=command
    )
